...
for (i in seq(1,loopEnd) ) {
  ...
  if(IS_RETRAIN_MODEL) {
    # mod.set <- linear(x.train, m=embedded_dim, d=delay, steps=futureSteps)
    #mod.set <- linear(x.train, m=embedded_dim, d=delay, steps=futureSteps, type="ADF")
    # mod.set <- linear(x.train, m=embedded_dim, d=delay, steps=futureSteps, type="diff")
    mod.set <- nnetTs(x.train, m=embedded_dim, d=delay, steps=futureSteps,size = 10)
  } else {
    mod.set$x = x.train;
  }
...
predictedValues = -avg_error+predictedValues;
